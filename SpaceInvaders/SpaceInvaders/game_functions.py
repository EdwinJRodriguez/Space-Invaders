...
def check_keydown_events(event, settings, ship, game):
    key = event.key
    if key == pg.K_SPACE: 
        ship.shooting = True
    if key == pg.K_ESCAPE:
...
def check_events(settings, ship, game):
    for event in pg.event.get():
        if event.type == pg.QUIT: sys.exit()
        elif event.type == pg.KEYDOWN: check_keydown_events(event=event, settings=settings, ship=ship, game = game)
